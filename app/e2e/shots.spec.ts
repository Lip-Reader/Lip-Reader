...
for (const [size, viewport] of Object.entries(SIZES)) {
  test(`screenshots ${size}`, async ({ page }) => {
    ...
    await page.screenshot({ path: `${OUT}/settings-hebrew-${size}.png` });
    await page.getByTestId("phrase-pain_hurts").click();
    await page.waitForTimeout(1200);
    await page.screenshot({ path: `${OUT}/settings-enroll-${size}.png` });
    await page.goto("/talk");
    await page.waitForTimeout(1500);
    await page.getByTestId("talk-button").click();
    await page.waitForTimeout(600);
    await page.getByTestId("stop-button").click();
    await page.getByTestId("candidate-0").waitFor();
    await page.screenshot({ path: `${OUT}/talk-candidates-${size}.png` });
    await page.evaluate(() => localStorage.removeItem("chaplin_settings"));
